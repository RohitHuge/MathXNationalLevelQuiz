...
export const setupRound3Sockets = (io, socket) => {
    ...
    socket.on('client:round3:buzzer_pressed', (data) => {
        ...
        console.log(`Current Lock State: ${gameState.buzzerLocked ? 'LOCKED (Dropping Queue)' : 'UNLOCKED (Accepting)'}`);

        const { teamId, timestamp } = data;

        // Always emit buzzer_hit so the sound plays on ALL clients, even when locked
        ...
        if (gameState.buzzerLocked) return; // Drop queue entry if locked, but sound already played
