...
export function Round3Client() {
    ...
    // Sound Effects
    const buzzerSound = React.useRef(new Audio('/sounds/buzzer_sound.mp3'));
    const questionDisplaySound = React.useRef(new Audio('/sounds/question_display.mp3'));
    const timerSound = React.useRef(new Audio('/sounds/timer.mp3'));
    const timerSoundPlayed = React.useRef(false); // Guard: only play once per countdown
    ...
    // Play question_display sound when a new question is shown (SR1-4 only, not SR5)
    const prevQuestionId = React.useRef(null);
    useEffect(() => {
        const state = gameState;
        if (!state) return;
        const { activeQuestion, activeSubRound } = state;
        if (!activeQuestion || activeSubRound === 5) return;
        if (activeQuestion.id !== prevQuestionId.current) {
            prevQuestionId.current = activeQuestion.id;
            questionDisplaySound.current.currentTime = 0;
            questionDisplaySound.current.play().catch(() => { });
        }
    }, [gameState?.activeQuestion?.id, gameState?.activeSubRound]);

    // Play timer.mp3 once when timer drops to 10 seconds
    useEffect(() => {
        const state = gameState;
        if (!state) return;
        const { timerTime, isTimerRunning } = state;
        if (isTimerRunning && timerTime === 10 && !timerSoundPlayed.current) {
            timerSoundPlayed.current = true;
            timerSound.current.currentTime = 0;
            timerSound.current.play().catch(() => { });
        }
        // Reset guard when timer resets above 10
        if (timerTime > 10) {
            timerSoundPlayed.current = false;
            timerSound.current.pause();
            timerSound.current.currentTime = 0;
        }
    }, [gameState?.timerTime, gameState?.isTimerRunning]);
